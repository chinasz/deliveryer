	...
	*/
	...
	class Store extends Auth{
		...
		public function income(){
			...
			$output['cashed'] = empty($output['cashed'])?'0.00':abs($output['cashed']);
			jsonReturn(0,'',$output);
		...
		public function feedetail(){
			...
			$where = ['uniacid'=>$this->uniacid,'sid'=>$this->store_id];
			if(!empty($type)) $where['trade_type'] = intval($type);
			$detail = pdo_getall('rhinfo_service_store_current_log',$where,$field);
			if(!empty($detail)){
		...
		public function accountlog(){
			$id = getvar('id');
			$field = ['trade_type','fee','amount','remark','extra'];
			$detail = pdo_get('rhinfo_service_store_current_log',['uniacid'=>$this->uniacid,'sid'=>$this->store_id,'id'=>intval($id)]);
			if(empty($detail)) jsonReturn(1,'记录不存在');
			if($detail['trade_type'] == 2){
				
				$cash_log = pdo_get('ims_rhinfo_service_store_getcash_log',['uniacid'=>$this->uniacid,'sid'=>$this->store_id,'id'=>$detail['extra']]);
			}
