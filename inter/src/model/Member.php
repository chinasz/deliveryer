...
*/
	...
	class Member extends \We7Table{
		...
		*/
		public function newMember($data){
			$member = [
				...
				'mobile' => empty($data['phone'])?'':$data['phone'],
				'avatar'=>empty($data['avatar'])?'':$data['avatar'],
				'mobile_audit' => 1,
				'nickname' => empty($data['nickname'])?'user'.intval(microtime(true)):$data['nickname'],
				'salt' => random(6, true),
				'addtime' => TIMESTAMP,
				'register_type' => 'app',
				'is_sys' => 2
		...
		**Time:9:17
		*/
		public function wechatMember($data){
			
			$member = $this->query->from($this->tableName)->where(['uniacid'=>$this->uniacid,'openid'=>$data['openid']])->get();
			if(empty($member)){
				$res = $this->newMember($data);
				$id = pdo_insertid();
			}else{
				$id = $member['id'];
			}
