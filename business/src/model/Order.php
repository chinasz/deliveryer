		...
		public function getOrder($sid,$page=1,$type='',$field=['o.*','m.nickname','m.avatar']){
			...
			if(!empty($orders)){
				foreach($orders as $k=>$order){
					...
					$order['goods'] = $this->query->from('rhinfo_service_order_stat')->select(['goods_num','goods_title','goods_unit_price','goods_price'])->where(['uniacid'=>$this->uniacid,'oid'=>$order['id']])->getall();
					if(!empty($order['status'])){
						$index = $order['status'];
						$order['status_cn'] = $order_status[$index]['text'];
					}
					$orders[$k] = $order;
				}	
